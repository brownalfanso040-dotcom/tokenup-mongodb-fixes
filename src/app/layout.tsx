import { Metadata } from 'next';
import Script from 'next/script';
import Header from '@/components/layout/Header';
import AppWalletProvider from '@/provider/AppWalletProvider';
import { StateContextProvider } from '@/provider/StateProvider';
import { NetworkProvider } from '@/context/NetworkContext';
import './globals.css';

export const metadata: Metadata = {
  title: 'Coin Generator',
  description: 'Generated by Gladwyn Anderson',
  applicationName: 'Solana Token Creation',
  generator: 'Next.js',
  authors: {
    name: 'CoinForge',
  },
  keywords: ['solana', 'token', 'creation', 'token 2022', 'launch', 'platform', 'coin', 'spl', 'spl-token', 'token extension', 'sol'],
  icons: {
    icon: '/Sol-Logo.jpg',
  },
  openGraph: {
    type: 'website',
    // url: 'https://coinforge.fun',
    title: 'Gladwyn Anderson',
    description: 'Generated by Gladwyn Anderson',
    siteName: 'Solana Token Creation Website',
    images: [
      {
        url: 'https://i.ibb.co/yFDyJnVD/coinforge.jpg',
      },
    ],
  },
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang='en'>
      <head>
        <title>CoinForge</title>
        <meta name='description' content='Generated by CoinForge' />
        <meta name='author' content='CoinForge' />
        <meta name='keywords' content='Solana, Token, Creation, Token 2022' />
        {/* eslint-disable-next-line @next/next/no-page-custom-font */}
        <link
          href='https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;700&display=swap'
          rel='stylesheet'
        />
        <Script async src='https://www.googletagmanager.com/gtag/js?id=G-15H4XSTRE5'></Script>
        <script
          id='google-analytics'
          dangerouslySetInnerHTML={{
            __html: `
window.dataLayer = window.dataLayer || [];
function gtag(){dataLayer.push(arguments);}
gtag('js', new Date());
gtag('config', 'G-15H4XSTRE5');
    `,
          }}
        />
      </head>
      <body
        className={`antialiased w-full bg-gradient-to-br from-gray-900 via-[#0c1527] to-main min-h-screen flex flex-col`}
      >
        <StateContextProvider>
          <NetworkProvider>
            <AppWalletProvider>
              <Header />
              <main className='flex-1'>{children}</main>
            </AppWalletProvider>
          </NetworkProvider>
        </StateContextProvider>
      </body>
    </html>
  );
}
